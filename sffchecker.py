...
opts_parser = argparse.ArgumentParser(
    description="Scan your calibre collection \
and try to find matches in SFF large collection."
)
opts_parser.add_argument(
    "-d",
    "--debug",
    help="add (a lot) of traces",
    dest="debug",
    action="store",
    default=False,
)
opts_parser.add_argument(
    "-v", "--verbose", help="add (a lot) of details", action="store_true", default=False
)
opts_parser.add_argument(
    "-D",
    "--display",
    help="linear or grouped display",
    dest="display",
...
opts_parser.add_argument("--sff", help="SFF directory path", dest="sff", action="store")
opts_parser.add_argument(
    "-A", "--author", help="1 specific author to look at", dest="author", action="store"
)
opts = None
...
def display_book(color, title):
    return """- {color}NEW{normal} - {title}""".format(
        color=color, normal=C_NORMAL, title=title
    )
...
def group_by_title(ltitles):
    ...
    def title_sort(a):
        ...
        if n is None:
            ...
            return n
    stitles = sorted(ltitles, key=lambda a: title_sort(a))
    for t, isuffix in itertools.groupby(stitles, iter_group_on_title):
        raw = list(isuffix)
        extra = None
...
def iter_group_on_collection(status_title):
    ...
    if l == 2:
        ...
        print(
            (
                "(error) splitted len's={0} for {1}, don't know what to do!".format(
                    l, title
                )
            )
        )
...
def group_by_collection(status_title):
    ugroup = {}
    for collection, ititles in itertools.groupby(
        status_title, iter_group_on_collection
    ):
        raw = list(ititles)
        for r in raw:
            title = None
            ...
            if collection == "__none__":
                ...
                number = grab_last_number(work[2])
                title = work[3].strip()

            if len(work) > 2 and title is not None:
                if collection in ugroup:
                    ugroup[collection].append(
                        {
                            "title": title,
                            "number": number,
                            "status": status,
    ...
    # pprint(ugroup)
    groups = [{"collection": c, "titles": group_by_title(t)} for c, t in ugroup.items()]
    return groups
...
def display_grouped(status_title):
    ...
    for group in groups:
        ...
        if collection == "__none__":
            ...
            if len(raw_title["suffix"]) == 1:
                ...
                title = "{0}.({1})".format(
                    raw_title["title"], "|".join(raw_title["suffix"])
                )
            if raw_title["status"]:
                print(("          {0}".format(display_book(C_GREEN, title))))
            else:
    ...
    for group in groups:
        ...
        if collection != "__none__":
            ...
            print(
                (
                    """    - {0} ({1} new + {2} update)""".format(
                        collection, cnt_new, cnt_upd
                    )
                )
            )
            for raw_title in group["titles"]:
                ...
                if raw_title["status"]:
                    print(
                        (
                            """        - {green}NEW{normal} - {number} - {title}""".format(
                                green=C_GREEN,
                                normal=C_NORMAL,
                    ...
                    print(
                        (
                            """        - {blue}UPD{normal} - {number} - {title}""".format(
                                blue=C_BLUE,
                                normal=C_NORMAL,
                                number=raw_title["number"],
                                title=title,
...
def match_authors(calibre_author, sff_author):
    ...
    if calibre_name == sff_name and calibre_fornames.replace(
        " ", ""
    ) == sff_name.replace(" ", ""):
        return True
...
def normalize_author(calibre_author):
    ...
    if len(parts) == 1:
        ...
        if lookslikeinitial(prenom):
            ...
            return [
                "{0}{1}".format(prenom_sff, nom),
                "{0}. {1}".format(prenom[0], nom),
            ]
        else:
            return [
                "{0} {1}".format(prenom, nom),
                "{0}. {1}".format(prenom[0], nom),
            ]
    else:
        ...
        if len(parts) == 3:
            ...
            if troisieme == "Von" or troisieme == "Van":
                return [
                    ...
                    "{0} {1}. {2} {3}".format(
                        prenom, second[0], troisieme.lower(), nom
                    ),
                    "{0} {1}. {2}. {3}".format(prenom, second[0], troisieme[0], nom),
                    "{0}. {1}. {2}. {3}".format(
                        prenom[0], second[0], troisieme[0], nom
                    ),
                    "{0}. {1}. {2} {3}".format(prenom[0], second[0], troisieme, nom),
                    "{0}. {1}. {2} {3}".format(
                        prenom[0], second[0], troisieme.lower(), nom
                    ),
...
def normalize_authors(calibre_authors):
    ...
    if len(authors) > 2:
        ...
        if cnt_1 > 1 and cnt_2 > 1:
            ...
            results = normalize_author(
                "{1} {0}".format(authors[0].strip(), authors[1].strip())
            )
    else:
...
if __name__ == "__main__":
    ...
    exact_authors, notfound_authors = lookup_match_authors(
        d_calibre, d_sff, opts.author
    )

    print(
        (
            "{0} authors in Calibre, {1} authors in new sff update, found {2} authors in both".format(
                len(d_calibre), len(d_sff), len(exact_authors)
            )
        )
    )
    if opts.debug:
